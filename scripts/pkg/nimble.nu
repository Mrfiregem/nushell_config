...
export def get-config [] {
    return {
        ...
        tasks: (get-tasks)
        paths: {
            nim: (which 'nim' | get 0?.path)
            nimble: (which 'nimble' | get 0?.path)
            nimpretty: (which 'nimpretty' | get 0?.path)
            nimsuggest: (which 'nimsuggest' | get 0?.path)
            nimgrep: (which 'nimgrep' | get 0?.path)
        }
    }
}
